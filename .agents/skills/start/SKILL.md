...
- **Session state?** Resolve identity from `git config user.name` via `.codex/team.json`,
  then check `team/session-state/{identity}/active.md`.
- **Memo inbox?** Glob `team/memo/{identity}/open/*.md` (exclude `.gitkeep`) to detect
  ...
  heading for dashboard preview. Protocol detail: `.codex/docs/team-memo-protocol.md`.

Compatibility note: Codex project paths use `.codex`; shared skills live in `.agents/skills`.
