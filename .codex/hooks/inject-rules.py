...
Reads stdin JSON from Codex, parses .codex/rules/*.md frontmatter
`paths:` glob list, matches against tool_input.file_path, and outputs
hookSpecificOutput.additionalContext if any rule matches.

Session-level marker in .codex/state/rules-injected/{session_id}/{rule_name}
prevents repeated injection within the same session.

See .codex/docs/rules-mechanism.md for the full convention.
"""
...
STATE_DIR = REPO_ROOT / ".codex" / "state" / "rules-injected"
LOG_FILE = REPO_ROOT / ".codex" / "state" / "inject-rules.log"
