...
├── .codex/                      # Codex hooks, agents, docs, rules
│   ├── hooks.json               # Codex hook wiring
│   ├── agents/                  # Codex specialist agent definitions
│   ├── docs/                    # Reference docs and templates
│   ├── rules/                   # Path-scoped editing rules
│   └── team.json                # Team identity map
├── .agents/                     # Codex skills
...
Identity is resolved from `git config user.name` via `.codex/team.json`.
...
- **`team/memo/`** 是跨开发者轻量 ping 机制 — sender 写到 `{recipient}/open/`，recipient `/start` 时自动 surface；处理完 `git mv` 到 `closed/`。详 `.codex/docs/team-memo-protocol.md`。

Directories 按需 lazy 创建。初始仅 `tools/`、`team/`、`art/`、`.codex/`、`docs/`、`design/`、`client/`、`prototypes/`、`plan/` 存在。
