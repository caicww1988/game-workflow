...
本机制在编辑指定 path 的文件时自动向 agent 注入对应规则。基于 Codex hooks 实现，**非官方 API**（属 PROJECT_NAME 自定义 convention）。
...
| 时机 | Hook | 行为 |
|---|---|---|
| **Session 启动** | `.codex/hooks/session-start.sh` 末尾段 | 全量加载 `.codex/rules/*.md` 完整内容到 SessionStart context，**首次编辑也有规则可遵守** |
| **每次 Edit / Write / MultiEdit 后** | `.codex/hooks/inject-rules.py`（PostToolUse） | 解析 frontmatter `paths:` glob，命中目标文件则注入对应 rule 到 `additionalContext` |

PostToolUse 是 Codex 官方仅支持 `additionalContext` 注入的 hook 事件（PreToolUse 不支持），所以注入是**事后的**。SessionStart 路径补充"首次编辑前空窗"。
...
```
.codex/state/rules-injected/{session_id}/{rule_name}
```

二次 Edit 同一 path → marker 已存在 → skip 注入。这控制 token 浪费，但仍保证规则在首次 Edit 时已生效。

`.codex/state/` 已加入 `.gitignore`。
...
1. **写规则文件** — `.codex/rules/{name}.md`，含 frontmatter `paths:`
...
直接 `git rm .codex/rules/{name}.md`。已注入过的 session 中残留的 marker 文件可不清理（无害）。
...
`.codex/state/inject-rules.log` 记录每次 hook 决策（命中 / skip / 异常）。
...
如要强制重新注入（用于测试）：删除 `.codex/state/rules-injected/{session_id}/` 即可。
...
| **frontmatter 协议是自定义 convention** | 非 Codex 官方 API，未来若有更优官方机制可弃用本 hook |
...
| **Hook 失败不阻断 Edit** | 异常 silent exit 0；timeout 8s 兜底；查 `.codex/state/inject-rules.log` 排查 |

---

## 历史背景

PROJECT_NAME 从上游模板 [`Claude-Code-Game-Studios`](https://github.com/) fork 来。上游 `.codex/rules/` 含 11 个文件 + frontmatter `paths:`，但**模板的 `rules-reference.md` 错误声称"automatically enforced when editing files in matching paths"**，实际 Codex 官方不支持该机制。本机制是 PROJECT_NAME 单边补全的真正实现（PostToolUse hook + SessionStart 兜底），不回流上游。
