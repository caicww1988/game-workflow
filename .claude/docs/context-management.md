...
Each developer maintains their own `team/session-state/{identity}/active.md` as a
living checkpoint (identity is resolved from git config via `.claude/team.json`).
...
### Memory Files (Per-Developer)

Beyond `active.md`, each developer can create custom `.md` files in their
`team/session-state/{identity}/` folder for persistent tracking:

- **TodoList**: `todo.md` — personal task tracking across sessions
- **Progress trackers**: `illustration-progress.md`, `model-tracker.md` — pipeline stage tracking
- **Notes**: `review-notes.md`, `sprint-notes.md` — cross-session reference

These files are git-tracked for cross-device sync. Hooks automatically:
- `session-start.sh`: lists memory files with line counts at session start
- `pre-compact.sh`: dumps first 30 lines of each before compaction

Naming convention: lowercase-kebab-case, descriptive of content.
...
- Reference to `team/session-state/{identity}/active.md` (read it to recover state)
...
**After compaction:** Read `team/session-state/{identity}/active.md` and any files being
